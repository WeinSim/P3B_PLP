...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Abbildungen/Graph_TV1.pdf}
    \caption{Fit der Paschen-Kurve an die gemessenen Werte (\coderef{tv1})}
    \label{graph_tv1}
\end{figure}
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{\coderef{tv1}}
    \label{fitparameters_tv1}
\end{table}

% TODO continue: A und B vertauschen
Die Funktion $f$ hat die Ableitung
\[
    f'(x) \wideeq \frac{
        A (\ln(B x) - C) - A x \frac{1}{B x} \cdot B
    }{
        \left( \ln(B x) - C \right)^2
    }
    \wideeq A \frac{\ln (B x) - C - 1}{\left( \ln(B x) - C \right)^2}
\]
Setzt man diese gleich Null, so erhält man für die $x$-Koordinate des Minimums
\begin{align*}
    A \frac{\ln (B x) - C - 1}{\left( \ln(B x) - C \right)^2} \wideeq& 0 \\
    \ln (B x) \wideeq& C + 1 \\
    x_0 \wideeq& \frac 1 B e^{C + 1}
\end{align*}
Daraus ergibt sich für die $y$-Koordinate des Minimums
\begin{align*}
    f(x_0) \wideeq \frac{A / B e^{C + 1}}{\ln (e^{C + 1}) - C}
    \wideeq \frac A B e^{C + 1}
\end{align*}
